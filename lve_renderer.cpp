...
namespace lve {
	...
	void LveRenderer::run(VkDevice device, LveSwapChain& swapChain, VkQueue graphicsQueue, VkQueue presentQueue, uint32_t& currentFrame,
		...
		recordCommandBuffer(commandBuffers[currentFrame], imageIndex,
			renderPass,swapChain.getSwapChainFrameBuffer(imageIndex),swapChain.getSwapChainExtent(),model,pipelineLayout,currentFrame,descriptorSets,compute);//记录命令缓冲区
	...
	void LveRenderer::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex,VkRenderPass renderPass,
		...
		const std::vector<VkDescriptorSet> descriptorSets, LveCompute& compute) {
		{
			...
			//绘制三角形
			model.draw_index_mode(commandBuffer);
